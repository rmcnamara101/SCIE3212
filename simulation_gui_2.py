...
class SimulationGUI:
    ...
    def setup_style(self):
        """Set up a modern, inspirational ttk style without changing plotting/data logic."""
        try:
            import tkinter as tk
        ...
        self.root.option_add("*TNotebook.TabPadding", [14, 8])
        self.root.option_add("*TLabelframe.LabelFont", "Helvetica 10 bold")
        # Palette
        bg = "#0f1226"          # deep space
        card = "#171a35"        # card surface
        ...
        style.configure("Headline.TLabel", background=bg, foreground=text, font=("Helvetica", 18, "bold"))
        style.configure("Subhead.TLabel", background=bg, foreground=sub, font=("Helvetica", 10))
        style.configure("Status.TLabel", background=card, foreground=sub, font=("Helvetica", 9))

        # Remember for later
        self._ui_colors = {"bg": bg, "card": card, "accent": accent, "text": text, "sub": sub}
    ...
    def create_brand_header(self, parent):
        ...
        title.pack(anchor="w")
        subtitle = ttk.Label(left, text="Analyse. Iterate. Discover patterns that move medicine forward.", style="Subhead.TLabel")
        subtitle.pack(anchor="w", pady=(2,0))

        right = ttk.Frame(header, style="Card.TFrame")
        right.pack(side=tk.RIGHT)
        ttk.Button(right, text="New Session", style="Accent.TButton", command=self.reload_current_file).pack(side=tk.LEFT, padx=4)
        ttk.Button(right, text="Open (Ctrl+O)", command=self.load_simulation_file).pack(side=tk.LEFT, padx=4)
