...
class SimulationAnalyzer:
    ...
    def plot_3d_at_step(self, step_index, levels=None):
        """Plot 3D isosurfaces of tumor fields at a specific time step.
        ...
        for cell_type, field in fields.items():
            ...
            if np.max(field) > level:  # Only plot if data exceeds isosurface level
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title(f'3D Tumor Visualization at Step {step_index}')
        plt.show()

    def plot_cross_section(self, step_index, plane='XY', index=None):
        """Plot 2D cross-sections of tumor fields at a specific time step.
...
if __name__ == '__main__':
    # Example usage
    analyzer = SimulationAnalyzer('data/test_sim_data.npz')
    analyzer.plot_volumes(smooth_window=7, normalize='initial')
    analyzer.plot_radius(smooth_window=7)
    analyzer.plot_3d_at_step(10)
    analyzer.plot_cross_section(10, plane='XY')
    analyzer.animate_cross_section(plane='XY', save_as='tumor_evolution.mp4')
