...
def run_and_save_cpp_sim(
    grid_shape=(100, 100, 100),
    steps=100,
    save_steps=10,
    dx=75,
    dt=None,  # Make dt adaptive
    save_dir="simulation_results",
    params=None
):
    """
    Run a simulation using the C++ implementation and save results.
    ...
    """
    print(f"Initializing simulation with grid shape: {grid_shape}")
    
    # Adjust dt based on grid size for stability
    if dt is None:
        grid_size = max(grid_shape)
        dt = 0.1 / (grid_size / 50)  # Scale dt with grid size
    
    print(f"Using dt = {dt} for grid size {grid_shape}")
    
    # Create save directory if it doesn't exist
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = os.path.join(save_dir, f"cpp_sim_{timestamp}")
    os.makedirs(save_path, exist_ok=True)
    
    # Initialize model with SphericalTumor initial conditions
    ...
    history_dict = {
        ...
        # Include metadata directly in history
        'Simulation Metadata': {
            "grid_shape": grid_shape,
            "steps": steps,
            "save_steps": save_steps,
    ...
    print("Simulation completed and saved successfully!")
    return save_path

if __name__ == "__main__":
    # Test with 100x100x100 grid with optimized parameters
    save_path = run_and_save_cpp_sim(
        grid_shape=(40, 40, 40),
        steps=100,
        save_steps=10,  # Save less frequently
        dx=20,
        dt=2,  # Slightly larger time step
        params={
            **SCIE3121_params,
        }
    )
