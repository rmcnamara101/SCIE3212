...
class TumorGrowthSimulator:
    ...
    def load_simulation_data(self, filepath):
        """
        Load simulation data from saved file using memory mapping to avoid loading
        entire file into memory (prevents crashes with large files).
        ...
        # Load data with memory mapping to avoid loading entire file into memory
        # This prevents crashes when loading large simulation files
        data = np.load(filepath, allow_pickle=True, mmap_mode='r')
        
        # Load metadata from pickle bytes (metadata is small, safe to load fully)
        import pickle
        metadata_bytes = data["metadata"].tobytes()
        metadata = pickle.loads(metadata_bytes)
        
        # Reconstruct simulation data structure
        # Keep arrays as memory-mapped views to avoid loading into memory
        simulation_data = {
            "metadata": metadata,
            "field_data": {
                "phi_hat": data["phi_hat"],  # Memory-mapped array
                "nutrient_fields": data["nutrient_fields"],  # Memory-mapped array
                "host_fields": data["host_fields"]  # Memory-mapped array
            },
            "performance": {
                "step_times": data["step_times"],  # Memory-mapped array
                "total_cells": data["total_cells"]  # Memory-mapped array
            },
            "_npz_file": data  # Keep reference to keep file open for memory mapping
        }
        
        # Add physics data if available - use lazy loading to avoid loading all at once
        if "pressure" in data:
            # Store the npz file reference and create a lazy accessor
            # This prevents loading all physics data into memory at once
            num_steps = len(data["pressure"])
...
class LazyPhysicsData:
    """
    Lazy loader for physics data to avoid loading all steps into memory at once.
    Only loads data when accessed, preventing memory crashes with large simulations.
    """
    def __init__(self, npz_file, num_steps):
        self._npz_file = npz_file
        self._num_steps = num_steps
        self._cache = {}  # Cache for accessed steps
        self._max_cache_size = 10  # Limit cache to prevent memory bloat
    
    def __len__(self):
        return self._num_steps
    
    def __getitem__(self, step_idx):
        """Get physics data for a specific step, with caching."""
        if step_idx in self._cache:
            return self._cache[step_idx]
        
        if step_idx >= self._num_steps:
            raise IndexError(f"Step index {step_idx} out of range (max: {self._num_steps - 1})")
        
        # Load only the requested step
        physics_data = {
            "pressure": self._npz_file["pressure"][step_idx],
            "velocity": self._npz_file["velocity"][step_idx],
            "energy_derivative": self._npz_file["energy_derivative"][step_idx],
            "mass_flux": self._npz_file["mass_flux"][step_idx]
        }
        
        # Add source terms if available
        if "source_terms" in self._npz_file:
            physics_data["source_terms"] = self._npz_file["source_terms"][step_idx]
        ...
        self._cache[step_idx] = physics_data
        return physics_data
    
    def __iter__(self):
        """Iterate over all steps (lazy loading)."""
